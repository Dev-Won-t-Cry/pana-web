...
const Nav = () => {
  ...
  return (
    <nav className="flex flex-col items-center space-y-8 bg-black/30 rounded-5xl p-8">
      {
        links.map(link => (
          <a key={link.href} href={link.href} className={`w-full border-2 border-black rounded-full text-center px-4 py-2 uppercase transition ease-in-out duration-300 font-bold ${pathname === link.href ? 'bg-dashboard-primary hover:bg-red-500' : 'bg-white hover:bg-gray-300'}`}>{link.label}</a>
        ))
      }
